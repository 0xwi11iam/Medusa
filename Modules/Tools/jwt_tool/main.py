...
def jwt_decode(token):
    ...
    try:
        parts = token.split(".")
        if len(parts) != 3: return "Not a valid JWT (needs 3 parts)"
        def _b64decode(s):
            s += "=" * (4 - len(s) % 4)
    ...
    except Exception as e: return f"JWT decode error: {e}"
...
def jwt_forge_hs256(payload_json, secret):
    ...
    sig = base64.urlsafe_b64encode(hmac.new(secret.encode(), f"{header_b64}.{body_b64}".encode(), hashlib.sha256).digest()).rstrip(b"=").decode()
...
def jwt_crack(token, wordlist_path="~/wordlists/jwt-secrets.txt"):
    ...
    if not os.path.exists(path): return f"Wordlist not found: {path}"
    parts = token.split(".")
    ...
    except: return "Invalid signature encoding"
    with open(path) as f:
        for line in f:
            secret = line.strip()
