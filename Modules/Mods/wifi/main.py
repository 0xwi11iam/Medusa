...
def _find_interface():
    ...
    # macOS
    try:
        r = subprocess.run("networksetup -listallhardwareports", shell=True,
    ...
    # Linux
    try:
        r = subprocess.run("iw dev 2>/dev/null | grep Interface | awk '{print $2}'", shell=True,
                           capture_output=True, text=True, timeout=5)
...
def _check_tool(tool_name):
    ...
    try:
        r = subprocess.run(f"which {shlex.quote(tool_name)}", shell=True,
