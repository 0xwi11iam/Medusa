...
APPROVALS_PATH = WORKSPACE_DIR / "approvals.json"
SESSION_PATH = WORKSPACE_DIR / "approved_tools.json"
_MAX_PENDING = 100
...
def record_pending(tool: str, args: dict) -> int | None:
    ...
    """
    try:
        data = _read(APPROVALS_PATH, {"next_id": 1, "items": []})
...
def list_approvals() -> list[dict]:
    return _read(APPROVALS_PATH, {"items": []}).get("items", [])
...
def decision_for(tool: str) -> str:
    ...
    sess = _read(SESSION_PATH, {"approved": [], "denied": []})
