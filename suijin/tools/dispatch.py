...
from suijin.tools.guardrails import _BLOCKED_PATTERNS, confirm_global_action, is_dangerous
from suijin.tools.http_tools import apply_patch, http_request, read_file, write_file
from suijin.tools.intel import (
    NOTES_DIR,
    NVD_BASE,
    _extract_cvss,
    _is_kev,
...
def _kb_read_tool(path: str) -> str:
    ...
    try:
        ...
        return f"Error: {e}"
    from suijin.tools.runtime import truncate
...
from suijin.tools.reporting import (
    _attack_tree,
    _diff_resp,
    _gen_report,
    _payload_gen,
...
def _recon_chain_route(target, config, ports=None):
    from suijin.tools.recon import recon_chain
...
"""
    ...
    from suijin.tools.availability import install_hint, missing_binaries
