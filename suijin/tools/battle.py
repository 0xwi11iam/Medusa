...
def _notify(title: str, message: str) -> None:
    ...
    try:
        from suijin.tools.notify import send
...
def run_battle(port: int = BLUE_LAB_PORT, watch_rounds: int = 40) -> dict:
    ...
    lab = subprocess.Popen(
        [
            sys.executable,
            str(Path(__file__).resolve().parents[2] / "suijin" / "lab" / "blue_target" / "vulnerable_app.py"),
        ],
    ...
    try:
        ...
        out_dir = Path(__file__).resolve().parents[2] / "suijin_agent" / "reports"
