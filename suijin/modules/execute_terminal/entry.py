    ...
    def start(self, ctx) -> None:
        ...
        for tool_name, (fn, params) in _load_tools().items():
            ...
            ctx.register_tool(
                tool_name,
                _bridge,
                description="Execute any shell command. Primary interface for CLI tools (gobuster, nmap, sqlmap, hydra, etc). Commands that modify the system (pip install, brew, sudo) require user approval.",
                owner="execute_terminal",
                params=params,
            )
            bridged += 1
        ctx.journal.append("execute_terminal", f"{bridged} tool(s) registered")
