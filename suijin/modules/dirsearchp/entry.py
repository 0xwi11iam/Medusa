...
class PackModule(Module):
    ...
    def start(self, ctx) -> None:
        ...
        for tool_name, fn in _load_tools().items():
            ...
            ctx.register_tool(tool_name, _bridge, description="Web path discovery via dirsearch.", owner="dirsearchp")
            bridged += 1
        ctx.journal.append("dirsearchp", f"{bridged} tool(s) registered")
