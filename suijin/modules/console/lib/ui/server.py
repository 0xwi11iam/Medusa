"""Flask backend for the Suijin WebUI. See suijin/ui/__init__.py."""
...
def __getattr__(name):
    if name in ("BLUE_KG_PATH", "BLUE_TARPIT_FILE", "BLUE_TRAFFIC_LOG"):
        return _const(name)
...
PKG_DIR = UI_DIR.resolve().parents[3]  # suijin/ package root
...
def build_snapshot() -> dict:
    snap: dict = {"ts": time.time(), "version": _version()}
    ...
    snap["tarpit"] = _read_json(Path(_const("BLUE_TARPIT_FILE")), {})
    traffic = _tail_jsonl(Path(_const("BLUE_TRAFFIC_LOG")), 200)
    ...
    kg = _read_json(Path(_const("BLUE_KG_PATH")), None)
    if kg:
        nodes = kg.get("nodes", {})
...
def create_app() -> Flask:
    ...
    def index():
        ...
        return (
            "<h1>Suijin UI</h1><p>Frontend not built. Run: <code>cd webui && npm install && npm run build</code></p>"
        ), 200
...
def run_server(port: int = 7800, open_browser: bool = True) -> int:
    ...
    print(f"Suijin UI v{_version()} — {url}  (Ctrl+C to stop)")
