    ...
    def start(self, ctx) -> None:
        ...
        for tool_name, fn in _load_tools().items():
            ...
            ctx.register_tool(
                tool_name,
                _bridge,
                description="Manual HTTP request with curl for quick targeted payload testing. Use for single-request manual testing only — NOT for scanning.",
                owner="curl",
            )
            bridged += 1
        ctx.journal.append("curl", f"{bridged} tool(s) registered")
