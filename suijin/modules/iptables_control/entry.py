...
class PackModule(Module):
    ...
    def start(self, ctx) -> None:
        ...
        for tool_name, fn in _load_tools().items():
            ...
            ctx.register_tool(tool_name, _bridge, description="Firewall rule management", owner="iptables_control")
            bridged += 1
        ctx.journal.append("iptables_control", f"{bridged} tool(s) registered")
