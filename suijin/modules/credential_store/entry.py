    ...
    def start(self, ctx) -> None:
        ...
        for tool_name, (fn, params) in _load_tools().items():
            ...
            ctx.register_tool(
                tool_name,
                _bridge,
                description="Persist and retrieve discovered credentials (passwords, API keys, tokens)",
                owner="credential_store",
                params=params,
            )
            bridged += 1
        ctx.journal.append("credential_store", f"{bridged} tool(s) registered")
