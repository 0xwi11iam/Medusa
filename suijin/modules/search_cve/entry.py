    ...
    def start(self, ctx) -> None:
        ...
        for tool_name, fn in _load_tools().items():
            ...
            ctx.register_tool(
                tool_name,
                _bridge,
                description="Query NIST NVD for CVEs by software and version. Returns CVSS scores, severity, CWE types, and exploit references.",
                owner="search_cve",
            )
            bridged += 1
        ctx.journal.append("search_cve", f"{bridged} tool(s) registered")
