...
class PackModule(Module):
    ...
    def start(self, ctx) -> None:
        ...
        for tool_name, fn in _load_tools().items():
            ...
            ctx.register_tool(tool_name, _bridge, description="Web technology stack fingerprinting", owner="whatweb")
            bridged += 1
        ctx.journal.append("whatweb", f"{bridged} tool(s) registered")
