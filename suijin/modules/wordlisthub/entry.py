    ...
    def start(self, ctx) -> None:
        ...
        for tool_name, (fn, params) in _load_tools().items():
            ...
            ctx.register_tool(
                tool_name,
                _bridge,
                description="Curated SecLists subset fetcher with checksums — fetch known-good wordlists into the workspace.",
                owner="wordlisthub",
                params=params,
            )
            bridged += 1
        ctx.journal.append("wordlisthub", f"{bridged} tool(s) registered")
