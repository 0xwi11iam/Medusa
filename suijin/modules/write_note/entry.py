    ...
    def start(self, ctx) -> None:
        ...
        for tool_name, fn in _load_tools().items():
            ...
            ctx.register_tool(
                tool_name,
                _bridge,
                description="Write timestamped engagement notes. MANDATORY — log every significant finding. Each engagement gets its own file.",
                owner="write_note",
            )
            bridged += 1
        ctx.journal.append("write_note", f"{bridged} tool(s) registered")
