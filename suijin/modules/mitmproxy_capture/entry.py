    ...
    def start(self, ctx) -> None:
        ...
        for tool_name, fn in _load_tools().items():
            ...
            ctx.register_tool(
                tool_name,
                _bridge,
                description="HTTP/HTTPS transparent proxy for traffic analysis",
                owner="mitmproxy_capture",
            )
            bridged += 1
        ctx.journal.append("mitmproxy_capture", f"{bridged} tool(s) registered")
