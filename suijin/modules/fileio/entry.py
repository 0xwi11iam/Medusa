    ...
    def start(self, ctx) -> None:
        ...
        for tool_name, fn in _load_tools().items():
            ...
            ctx.register_tool(
                tool_name,
                _bridge,
                description="Read and write files scoped to suijin_agent/ workspace by default. Absolute paths allowed.",
                owner="fileio",
            )
            bridged += 1
        ctx.journal.append("fileio", f"{bridged} tool(s) registered")
