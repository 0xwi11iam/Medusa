...
def graphql_introspect(url: str = "") -> str:
    ...
    try:
        r = requests.post(url.strip(), json=_INTROSPECTION, timeout=_T, headers={**_UA, "Content-Type": "application/json"})
    ...
    fields = {t["name"]: [f["name"] for f in (t.get("fields") or [])][:8] for t in types if t.get("fields") and t["name"] in interesting[:12]}
    out = [f"INTROSPECTION ALLOWED: {len(types)} types, query={schema.get('queryType')}, mutation={schema.get('mutationType')}"]
