...
class PackModule(Module):
    ...
    def start(self, ctx) -> None:
        ...
        for tool_name, fn in _load_tools().items():
            ...
            ctx.register_tool(tool_name, _bridge, description="Passive subdomain discovery tool", owner="subfinder")
            bridged += 1
        ctx.journal.append("subfinder", f"{bridged} tool(s) registered")
