    ...
    def start(self, ctx) -> None:
        ...
        for tool_name, (fn, params) in _load_tools().items():
            ...
            ctx.register_tool(
                tool_name,
                _bridge,
                description="Agent-defined custom commands: define named command templates, run them, persist across sessions.",
                owner="cmdsmith",
                params=params,
            )
            bridged += 1
        ctx.journal.append("cmdsmith", f"{bridged} tool(s) registered")
