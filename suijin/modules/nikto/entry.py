    ...
    def start(self, ctx) -> None:
        ...
        for tool_name, fn in _load_tools().items():
            ...
            ctx.register_tool(
                tool_name,
                _bridge,
                description="Web server vulnerability scanner. Finds outdated software, misconfigurations, and known vulnerable files.",
                owner="nikto",
            )
            bridged += 1
        ctx.journal.append("nikto", f"{bridged} tool(s) registered")
