    ...
    def start(self, ctx) -> None:
        ...
        for tool_name, fn in _load_tools().items():
            ...
            ctx.register_tool(
                tool_name,
                _bridge,
                description="Persistent target constraints. Check before generating payloads, record findings after verification.",
                owner="knowledge_graph",
            )
            bridged += 1
        ctx.journal.append("knowledge_graph", f"{bridged} tool(s) registered")
