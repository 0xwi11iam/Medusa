    ...
    def start(self, ctx) -> None:
        ...
        for tool_name, (fn, params) in _load_tools().items():
            ...
            ctx.register_tool(
                tool_name,
                _bridge,
                description="Bug-bounty scope scraper (bbscope-style): pull program scopes from HackerOne, Bugcrowd, YesWeHack, Intigriti, Immunefi. SSL-verified, paginated, stealth identity.",
                owner="bugscope",
                params=params,
            )
            bridged += 1
        ctx.journal.append("bugscope", f"{bridged} tool(s) registered")
