...
def cache_probe(url: str = "") -> str:
    ...
    try:
        ...
        return f"Error: {e}"
    cache_hdrs = {k: v for k, v in r1.headers.items() if k.lower() in ("x-cache", "cf-cache-status", "age", "x-varnish", "x-drupal-cache", "x-proxy-cache", "via", "etag")}
    if cache_hdrs:
    ...
    if same_body and cached:
        out.append("body stable + age/HIT present: cached layer exists — test unkeyed inputs (headers like X-Forwarded-Host) for poisoning")
    # unkeyed reflection
    try:
