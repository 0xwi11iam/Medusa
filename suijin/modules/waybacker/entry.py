...
class PackModule(Module):
    ...
    def start(self, ctx) -> None:
        ...
        for tool_name, fn in _load_tools().items():
            ...
            ctx.register_tool(
                tool_name, _bridge, description="Wayback Machine URL history via the public CDX API.", owner="waybacker"
            )
            bridged += 1
        ctx.journal.append("waybacker", f"{bridged} tool(s) registered")
