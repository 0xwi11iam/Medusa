"""Pure-Python oracles for the compiled core (native/suijin-core).

These reimplement resolve_dag and check_paths EXACTLY as the Rust crate
does — same canonical JSON output — and are:
  1. the permanent fallback when the compiled wheel is absent, and
  2. the test oracle: CI compares both implementations over generated
     manifest trees and asserts canonical-JSON equality.

Pure stdlib. Deliberately standalone: no imports from the rest of the
kernel, so it can be diffed line-by-line against src/lib.rs.
"""
