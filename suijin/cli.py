...
def run_version() -> int:
    ...
    try:
        ...
        pass
    print(f"suijin {VERSION}" + (f'  "{codename}"' if codename else "") + (f"  ({release})" if release else ""))
...
def run_ui(args) -> int:
    ...
    from suijin.ui.server import run_server
...
def run_selftest() -> int:
    ...
    print("Suijin selftest v" + VERSION)
    print("-" * 56)
    failed = 0
...
def main(argv=None):
    ...
    parser.add_argument("--version", action="version", version=f"suijin {VERSION}")
