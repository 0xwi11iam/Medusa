...
TOOL_DESCRIPTIONS = {
    ...
    "find_wordlist": "Find SecLists wordlists by keyword and extract them into medusa_agent/wordlists/. Args: keyword, extract?",
    "extract_payloads": "Extract runnable code blocks from KB docs into medusa_agent/payloads/. Args: keyword, max_payloads?",
    "kb_stats": "Knowledge base inventory: per-source doc counts, build age, failed sources.",
    "wordlist_tool": "Merge/dedupe/length-filter wordlists into medusa_agent/wordlists/. Args: action(dedupe|merge|filter), files[], out?, min_len?, max_len?",
    "mine_failures": "Cluster failure_db.json entries into technique/reason patterns to avoid. Args: max_clusters?",
    "anonymize_report": "Scrub IPs/emails/tokens/keys from a report into medusa_agent/reports/anonymized/. Args: file_path.",
