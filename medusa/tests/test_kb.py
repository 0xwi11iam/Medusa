...
from medusa import kb as kbmod
from medusa.kb import compile_kb, iter_docs, kb_status
from medusa.tools.intel import _fts_match_expr
...
class TestSearchKbTool:
    def test_disabled_when_not_built(self, monkeypatch):
        ...
        assert "medusa pull kb" in result
...
class TestRepoAnchoredPaths:
    """KB artifacts must ALWAYS live inside the medusa-security repo folder —
    never CWD-dependent, never scattered into $HOME or /tmp."""

    def test_paths_anchored_to_package_dir(self):
        pkg_dir = kbmod.Path(kbmod.__file__).resolve().parent  # <repo>/medusa
...
class TestCatalogFeatureGating:
    """`medusa pull kb` ENABLES knowledge base features; without it they are
    ...
    def test_catalog_lists_disabled_when_no_db(self, monkeypatch):
        ...
        assert "medusa pull kb" in catalog
...
class TestWorkspaceIntegrity:
    """The agent workspace (medusa_agent/) is sacred — KB artifacts must
    never leak into it, and its anchoring must not change."""

    def test_workspace_anchored_and_separate_from_kb(self):
        from medusa.tools.workspace import PROJECT_DIR, WORKSPACE_DIR
