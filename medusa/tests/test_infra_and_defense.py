...
from medusa.infra import output_offload as oo
from medusa.infra.tool_offload_policy import get_offload_mode
...
class TestFirewall:
    def test_validate_ip_accepts_valid(self):
        from medusa.core.blue.defense import firewall as fw
...
class TestTailFile:
    def test_yields_appended_lines(self, tmp_path):
        from medusa.tools.housekeeping import tail_file
...
class TestMsfCheck:
    def test_unavailable_without_rpc_or_console(self, monkeypatch):
        from medusa.tools import metasploit as msf
