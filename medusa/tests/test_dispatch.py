...
class TestPureHelpers:
    def test_truncate_short(self):
        from medusa.tools.dispatch import truncate
...
class TestFileOps:
    def test_read_file_missing(self):
        from medusa.tools.dispatch import read_file

        # Use an allowlisted path (/tmp) that doesn't exist
        result = read_file("/tmp/medusa_definitely_missing_file_xyz.txt")
...
class TestCVSSHelpers:
    def test_is_kev_true(self):
        from medusa.tools.dispatch import _is_kev
    ...
    def test_extract_cvss_v31(self):
        from medusa.tools.dispatch import _extract_cvss
...
class TestJobTracking:
    def test_job_status_unknown(self):
        from medusa.tools.dispatch import _job_status

        result = _job_status("nonexistent-job-id")
        assert isinstance(result, str)

    def test_job_list(self):
        from medusa.tools.dispatch import _job_list

        result = _job_list()
        assert isinstance(result, str)

    def test_job_cancel_unknown(self):
        from medusa.tools.dispatch import _job_cancel
...
class TestRouteTool:
    ...
    def test_get_tool_catalog_is_markdown(self):
        from medusa.tools.dispatch import get_tool_catalog
