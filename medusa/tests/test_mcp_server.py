"""Tests for medusa/mcp_server.py — the MCP sidecar bridging TUI to backend."""
import json
import subprocess
import sys
...
class TestMCPProtocol:
    def test_initialize(self):
        ...
        assert result["serverInfo"]["name"] == "medusa"
    ...
    def test_tools_list(self):
        ...
        assert {"medusa_tool", "execute_terminal", "medusa_detect",
                "medusa_kg_attacker", "medusa_status"} <= names
    ...
    def test_per_tool_registry(self):
        ...
        assert {"nmap_scan", "gobuster_dir", "sqlmap_scan", "write_note",
                "claim_flag", "search_cve"} <= set(tools)
    ...
    def test_named_tool_call(self):
        ...
        assert "[medusa] tool: claim_flag" in text

    def test_status_reports_tool_count(self):
        responses = _call_server([
            {"jsonrpc": "2.0", "id": 1, "method": "tools/call",
             "params": {"name": "medusa_status", "arguments": {}}},
        ])
...
class TestMCPTools:
    def test_medusa_status(self):
        responses = _call_server([
            {"jsonrpc": "2.0", "id": 1, "method": "tools/call",
             "params": {"name": "medusa_status", "arguments": {}}},
        ])
        data = json.loads(responses[0]["result"]["content"][0]["text"])
        assert data["server"] == "medusa"
        assert data["version"]

    def test_medusa_detect_sqli(self):
        responses = _call_server([
            {"jsonrpc": "2.0", "id": 1, "method": "tools/call",
             "params": {"name": "medusa_detect", "arguments": {"request": {
                 "method": "POST", "path": "/auth/login",
                 "body": "admin' OR '1'='1", "ip": "127.0.0.1"}}}},
        ])
        data = json.loads(responses[0]["result"]["content"][0]["text"])
        assert data["score"] >= 5
        assert any("SQL" in p[0] for p in data["patterns"])

    def test_medusa_detect_clean(self):
        responses = _call_server([
            {"jsonrpc": "2.0", "id": 1, "method": "tools/call",
             "params": {"name": "medusa_detect", "arguments": {"request": {
    ...
    def test_execute_terminal(self):
        responses = _call_server([
            {"jsonrpc": "2.0", "id": 1, "method": "tools/call",
             "params": {"name": "execute_terminal",
                        "arguments": {"cmd": "echo mcp_test_ok"}}},
        ])
        text = responses[0]["result"]["content"][0]["text"]
    ...
    def test_kg_attacker_empty(self):
        responses = _call_server([
            {"jsonrpc": "2.0", "id": 1, "method": "tools/call",
             "params": {"name": "medusa_kg_attacker", "arguments": {"ip": "1.2.3.4"}}},
        ])
        data = json.loads(responses[0]["result"]["content"][0]["text"])
    ...
    def test_medusa_tool_routes(self):
        """Generic dispatch through route_tool (claim_flag is pure)."""
        responses = _call_server([
            {"jsonrpc": "2.0", "id": 1, "method": "tools/call",
             "params": {"name": "medusa_tool", "arguments": {
                 "tool_name": "claim_flag", "args": {"flag": "FLAG{mcp_test}"}}}},
        ])
        text = responses[0]["result"]["content"][0]["text"]
