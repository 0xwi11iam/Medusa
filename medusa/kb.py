"""medusa/kb.py — downloadable security knowledge base, compiled to SQLite.

`medusa pull kb` downloads pure-markdown / text knowledge bases (HackTricks,
...
at medusa/kb.sqlite3. The DB never ships with the repo — users build it on
demand. The agent queries it through the `search_kb` tool (tools/intel.py).

Storage layout:
    medusa/kb_cache/    raw downloaded tarballs (skipped on re-pull)
    medusa/kb.sqlite3   compiled FTS5 database (consumed by search_kb)
...
BASE_DIR = Path(__file__).resolve().parent  # medusa/
...
def _default_session():
    from medusa.tools.runtime import global_session
