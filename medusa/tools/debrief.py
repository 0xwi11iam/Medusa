"""Engagement debrief — analytics over medusa_agent/audit_trails/*.json.

`medusa debrief` answers: what did we run, what worked, what did it cost,
...
def load_audits(audit_dir: Path | None = None) -> list[dict]:
    ...
    d = Path(audit_dir) if audit_dir else WORKSPACE_DIR / "audit_trails"
    out: list[dict] = []
    if not d.is_dir():
        return out
...
def render_debrief(trails: list[dict], verbose: bool = False) -> str:
    ...
    if not trails:
        return (
            ...
            "(medusa → Red Team). Artifacts land in medusa_agent/audit_trails/."
        )
    lines: list[str] = []
    stats = [engagement_stats(t) for t in trails]
