...
extract_payloads pull code blocks out of KB docs into medusa_agent/payloads/
...
from medusa.kb import CACHE_DIR, DB_PATH, SOURCES
...
def find_wordlist(keyword: str, extract: bool = True) -> str:
    ...
    medusa_agent/wordlists/ (the DB copy may be truncated for indexing).
    """
    keyword = (keyword or "").strip()
    if not keyword:
        return "Error: keyword required (e.g. 'directory', 'password', 'usernames', 'api')."
    if not DB_PATH.exists():
        return "Knowledge base DISABLED. Run 'medusa pull kb --sources seclists' to get the wordlists, then retry."
    try:
    ...
    if not rows:
        return (
            ...
            "pulled — run 'medusa pull kb --sources seclists' (~300 MB)."
        )
    ...
    for path, size in rows:
        ...
        lines.append(note)
    if extracted:
        lines.append("\nUse directly: ffuf -w medusa_agent/wordlists/<file> ...")
    else:
        lines.append("\nPass extract=true (default) to materialize files into medusa_agent/wordlists/.")
    return "\n".join(lines)
...
def _extract_seclists_file(rel_path: str) -> str | None:
    ...
    try:
        ...
        out_dir = resolve_workspace_path("wordlists")
        out_dir.mkdir(parents=True, exist_ok=True)
        out = out_dir / base
        out.write_bytes(data)
        return str(out.relative_to(WORKSPACE_DIR))
...
def kb_stats() -> str:
    ...
    if not DB_PATH.exists():
        return "Knowledge base DISABLED — ask the operator to run 'medusa pull kb'. Use web_search meanwhile."
    from medusa.kb import kb_status

    st = kb_status()
    if not st:
        return "Knowledge base DB present but unreadable — rebuild with 'medusa pull kb --force'."
    lines = [
    ...
    for name in sorted(st.get("failed", {})):
        lines.append(f"  {name:12} FAILED at last build — retry: medusa pull kb --sources {name}")
...
def suggest_exploit(service: str, version: str = "") -> str:
    ...
    if not DB_PATH.exists():
        return (
            "Knowledge base DISABLED. Run 'medusa pull kb' first, or use web_search. "
...
def extract_payloads(keyword: str, max_payloads: int = 10) -> str:
    """Pull runnable code blocks from matching KB docs into medusa_agent/payloads/."""
    keyword = (keyword or "").strip()
    if not keyword:
        return "Error: keyword required (e.g. 'reverse shell bash', 'sqli union bypass')."
    if not DB_PATH.exists():
        return "Knowledge base DISABLED. Run 'medusa pull kb' first."
    from medusa.tools.intel import search_kb
    ...
    out_dir = resolve_workspace_path("payloads")
    out_dir.mkdir(parents=True, exist_ok=True)
    written = []
...
def wordlist_tool(action: str, files: list | None = None, out: str = "", min_len: int = 1, max_len: int = 256) -> str:
    """Merge / dedupe / length-filter wordlists into medusa_agent/wordlists/.
    ...
    if not files:
        return "Error: files required — paths inside medusa_agent/ (e.g. wordlists/a.txt)."
    if not out:
    ...
    target = resolve_workspace_path(out)
    target.parent.mkdir(parents=True, exist_ok=True)
    target.write_text("\n".join(words) + "\n")
    return (
        f"{action}: {len(files)} file(s) -> {len(words):,} words "
        f"({n_raw - len(words):,} dropped) -> {target.relative_to(WORKSPACE_DIR)}"
    )
...
def mine_failures(max_clusters: int = 5) -> str:
    ...
    db = WORKSPACE_DIR / "failure_db.json"
    if not db.exists():
        return "No failure history yet (medusa_agent/failure_db.json). Record failures with record_finding as you go."
    try:
...
def anonymize_report(file_path: str) -> str:
    ...
    Writes the cleaned copy to medusa_agent/reports/anonymized/<name>.
    Flags (FLAG{...}) and localhost are preserved. Returns the output path
    plus a replacement count.
    """
    src = resolve_workspace_path(file_path)
    try:
    ...
    out_dir = resolve_workspace_path("reports") / "anonymized"
    out_dir.mkdir(parents=True, exist_ok=True)
    out = out_dir / src.name
    out.write_text(text)
    total = sum(counters.values())
    return f"Anonymized {total} item(s) {counters} -> {out.relative_to(WORKSPACE_DIR)}"
