...
medusa_agent/credentials.vault.json holds PBKDF2-HMAC-SHA256 derived
...
CLI: medusa creds init|list|add|get|export|--redact. The plaintext
...
def add_credential(
    ...
    if not vault_exists():
        return "No vault — run: medusa creds init"
...
def list_credentials(passphrase: str, reveal: bool = False) -> str:
    ...
    if not reveal:
        lines.append("\n(values hidden — `medusa creds list --reveal`)")
    return "\n".join(lines)
