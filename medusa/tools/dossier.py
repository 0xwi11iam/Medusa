...
CLI: `medusa dossier <target>`. Agent tool: `target_dossier(target)` — the
...
RED_KG_PATH = WORKSPACE_DIR.parent / "medusa" / "intel" / "knowledge_graph.json"
...
def build_dossier(target: str, workspace: Path | None = None, red_kg: Path | None = None) -> dict:
    ...
    kg_path = Path(red_kg) if red_kg else RED_KG_PATH
