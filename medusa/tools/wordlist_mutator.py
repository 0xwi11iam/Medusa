...
def mutate_wordlist(
    ...
    """Expand seed words into a mutated wordlist at medusa_agent/<out>."""
    ...
    target.write_text("\n".join(words) + "\n")
    try:
        shown = str(target.resolve().relative_to(WORKSPACE_DIR.resolve()))
    except ValueError:
        shown = str(target)
    return (
...
def cewl_words(url: str, out: str | None = None, min_len: int = 3, max_len: int = 24, session=None) -> str:
    ...
    target.parent.mkdir(parents=True, exist_ok=True)
    target.write_text("\n".join(words) + "\n")
    return f"harvested {len(words):,} word(s) from {url} -> {target.relative_to(WORKSPACE_DIR)}"
