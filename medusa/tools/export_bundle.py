...
`medusa export` packs everything an engagement produced into one zip with a
...
def build_bundle(
    ...
    red_kg = Path(red_kg_path) if red_kg_path else ws.parent / "medusa" / "intel" / "knowledge_graph.json"
    blue_kg = Path(blue_kg_path) if blue_kg_path else Path(BLUE_KG_PATH)
    cfg = Path(config_path) if config_path else ws.parent / "medusa" / "config.json"

    ts = datetime.now(timezone.utc)
    if out_path is None:
        out_dir = ws / "exports"
        out_dir.mkdir(parents=True, exist_ok=True)
        out_path = out_dir / f"medusa_bundle_{ts.strftime('%Y%m%d_%H%M%S')}.zip"
