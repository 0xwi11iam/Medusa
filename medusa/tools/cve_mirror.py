...
`medusa pull cve` downloads the Known Exploited Vulnerabilities catalog
(a public JSON feed, no auth) into medusa/cve_cache/kev.json. search_cve
...
def format_kev_results(vulns: list[dict], software: str) -> str:
    ...
    lines.append("\n(NVD unreachable — results from the local CISA KEV mirror; refresh with: medusa pull cve)")
    return "\n".join(lines)
