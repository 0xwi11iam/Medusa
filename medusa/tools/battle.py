...
`medusa battle` boots the blue_target lab, clears blue-team state, then runs
...
from medusa.core.constants import BLUE_KG_PATH, BLUE_LAB_PORT, BLUE_TARPIT_FILE, BLUE_TRAFFIC_LOG
...
def run_battle(port: int = BLUE_LAB_PORT, watch_rounds: int = 40) -> dict:
    ...
    lab = subprocess.Popen(
        [
            sys.executable,
            str(Path(__file__).resolve().parents[2] / "medusa" / "lab" / "blue_target" / "vulnerable_app.py"),
        ],
    ...
    try:
        ...
        out_dir = Path(__file__).resolve().parents[2] / "medusa_agent" / "reports"
