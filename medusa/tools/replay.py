...
`medusa replay` renders a live timeline in the terminal: play/pause with
...
from medusa.tools.debrief import load_audits
...
def run_replay(trail: dict) -> None:
    ...
    console.print(
        ...
        f"export the transcript with: medusa replay --file "
...
def pick_engagement(audit_dir: Path | None = None) -> dict | None:
    ...
    if not trails:
        console.print(
            "[yellow]No replayable engagements — audit trails with iterations live in medusa_agent/audit_trails/.[/]"
        )
        return None
