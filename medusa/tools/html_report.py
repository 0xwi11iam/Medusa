...
def export_html(findings: list, engagement_name: str, cost_usd: float = 0.0, trace_count: int = 0, output_path: str = None) -> str:
    ...
    for i, f in enumerate(findings[:50], 1):
        ...
        color = {"CRITICAL":"#7b0000","HIGH":"#cc0000","MEDIUM":"#cc6600","LOW":"#336600","INFO":"#336699"}.get(sev,"#333")
        rows += f"<tr><td>{i}</td><td style='color:{color}'><b>{sev}</b></td><td>{f.get('type','?')}</td><td>{f.get('endpoint','?')}</td><td>{f.get('description','')[:200]}</td></tr>"
    html = f"""<!DOCTYPE html><html><head><title>Medusa Report — {engagement_name}</title>
...
</style></head><body><h1>Medusa Engagement Report</h1>
