"""
Medusa Agent Graph — LangGraph state machine for autonomous red teaming.
...
class MedusaAgentGraph:
    """LangGraph-based autonomous red team agent.

    Usage:
        agent = MedusaAgentGraph(generate_fn=providers.generate_async,
    ...
    async def _think(self, state: dict) -> dict:
        try:
            ...
            if iteration > 0 and iteration % supervisor_interval == 0:
                try:
                    from medusa.core.supervisor import analyze_trace, analyze_trace_with_llm
                    ...
                    if guidance:
                        ...
                        try:
                            llm_guidance = await analyze_trace_with_llm(
                                trace, state, self.generate_fn
                            )
                            if llm_guidance:
            ...
            if iteration > 0 and iteration % 4 == 0:
                try:
                    from medusa.intel.oracle import detect_anomaly, generate_hypotheses_async
                    trace = result.get("execution_trace", state.get("execution_trace", []))
                    if trace:
                        last_step = trace[-1]
                        tool_output = str(last_step.get("tool_output", ""))
                        if detect_anomaly(tool_output):
                            hypotheses = await generate_hypotheses_async(
                                tool_output, state, self.generate_fn
                            )
                            if hypotheses:
                                result.setdefault("messages", []).append({
                                    "role": "user",
            ...
            if iteration > 0 and iteration % 7 == 0:
                try:
                    ...
                    trace = result.get("execution_trace", state.get("execution_trace", []))
                    recent_actions = [
                        f"{s.get('tool_name', '?')}: {str(s.get('thought', ''))[:100]}"
                        for s in trace[-10:]
                    ]
            ...
            return {
                ...
                "messages": [{"role": "user", "content": f"SYSTEM: think node crashed: {e}. The next action should use 'complete' to report the error."}],
    ...
    async def _execute_tool(self, state: dict) -> dict:
        try:
            ...
            return {
                ...
                "messages": [{"role": "user", "content": f"SYSTEM: execute_tool node crashed: {e}. Continue with next action."}],
            }
