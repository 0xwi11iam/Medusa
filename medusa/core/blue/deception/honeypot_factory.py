...
def generate_honeypot_response(endpoint: dict) -> dict:
    ...
    if "admin" in path.lower():
        return {"status": 200, "body": json.dumps({"users":[{"id":99,"name":"admin_backup","role":"admin","api_key":"fake_canary_"+random.randint(1000,9999).__str__()}]})}
    if "api" in path.lower():
        return {"status": 200, "body": json.dumps({"data":[],"meta":{"total":0,"debug_token":"canary_"+random.randint(1000,9999).__str__()}})}
