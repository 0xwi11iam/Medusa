...
export function cliErrorMessage(input: unknown): string | undefined {
  ...
  if (model) {
    ...
    return [
      ...
      "Try: `medusa models` to list available models",
      "Or check your config (medusa.json) provider/model names",
    ].join("\n")
  }
  ...
  if (isRecord(input) && named(input, "MCPFailed")) {
    ...
    return `MCP server "${name}" failed. Note, medusa does not support MCP authentication yet.`
  }
  return undefined
}
