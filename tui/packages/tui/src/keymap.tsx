...
export const MEDUSA_BASE_MODE = "base"
export const COMMAND_PALETTE_COMMAND = "command.palette.show"

const MEDUSA_MODE_KEY = "medusa.mode"
...
export function createOpencodeModeStack(keymap: OpenTuiKeymap) {
  keymap.setData(MEDUSA_MODE_KEY, MEDUSA_BASE_MODE)

  const offFields = keymap.registerLayerFields({
    mode(value, ctx) {
      ctx.require(MEDUSA_MODE_KEY, value)
    },
  })
  ...
  const update = () => {
    keymap.setData(MEDUSA_MODE_KEY, stack.at(-1)?.mode ?? MEDUSA_BASE_MODE)
  }

  const stackApi = {
    current() {
      return stack.at(-1)?.mode ?? MEDUSA_BASE_MODE
    },
    push(mode: string) {
      if (disposed) return () => {}
    ...
    dispose() {
      ...
      stack.length = 0
      offFields()
      keymap.setData(MEDUSA_MODE_KEY, undefined)
