...
describe("util.process", () => {
  ...
  test("merges environment overrides", async () => {
    const out = await Process.run(node('process.stdout.write(process.env.MEDUSA_TEST ?? "")'), {
      env: {
        MEDUSA_TEST: "set",
      },
    })
  ...
  test("uses shell in run on Windows", async () => {
    ...
    const out = await Process.run(["set", "MEDUSA_TEST_SHELL"], {
      shell: true,
      env: {
        MEDUSA_TEST_SHELL: "ok",
      },
    })

    expect(out.code).toBe(0)
    expect(out.stdout.toString()).toContain("MEDUSA_TEST_SHELL=ok")
  })
