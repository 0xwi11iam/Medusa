...
const cleanState = Effect.gen(function* () {
  ...
  delete process.env.MEDUSA_CONFIG
  delete process.env.MEDUSA_TUI_CONFIG
...
it.instance("project config takes precedence over MEDUSA_TUI_CONFIG (matches MEDUSA_CONFIG)", () =>
  withCleanState(
    Effect.gen(function* () {
      const fs = yield* FSUtil.Service
      const test = yield* TestInstance
...
it.instance("MEDUSA_TUI_CONFIG provides settings when no project config exists", () =>
  withCleanState(
    Effect.gen(function* () {
      const fs = yield* FSUtil.Service
      const test = yield* TestInstance
...
it.instance("does not derive tui path from MEDUSA_CONFIG", () =>
  withCleanState(
    Effect.gen(function* () {
      const fs = yield* FSUtil.Service
      const test = yield* TestInstance
