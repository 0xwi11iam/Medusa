...
import { NamedError } from "@medusa-ai/core/util/error"
...
import { CrossSpawnSpawner } from "@medusa-ai/core/cross-spawn-spawner"
...
import { ProjectV2 } from "@medusa-ai/core/project"
...
const managedConfigDir = process.env.MEDUSA_TEST_MANAGED_CONFIG_DIR!
const originalTestToken = process.env.TEST_TOKEN
const originalConsoleToken = process.env.MEDUSA_CONSOLE_TOKEN
...
afterEach(async () => {
  ...
  if (originalConsoleToken === undefined) delete process.env.MEDUSA_CONSOLE_TOKEN
  else process.env.MEDUSA_CONSOLE_TOKEN = originalConsoleToken
  await clear(true)
})
...
it.effect("does not create global config when MEDUSA_CONFIG_DIR is set", () =>
  Effect.gen(function* () {
...
const accountTokenIt = configIt({
  account: Layer.mock(Account.Service)({
    ...
    config: () =>
      Effect.succeed(
        Option.some({
          provider: { opencode: { options: { apiKey: "{env:MEDUSA_CONSOLE_TOKEN}" } } },
        }),
      ),
...
it.effect("does not try to install dependencies in read-only MEDUSA_CONFIG_DIR", () =>
  Effect.gen(function* () {
    if (process.platform === "win32") return
    ...
    yield* withProcessEnv("MEDUSA_CONFIG_DIR", readonly, Config.use.get().pipe(provideInstanceEffect(dir)))
  }).pipe(Effect.provide(testInstanceStoreLayer), Effect.provide(LayerNode.compile(CrossSpawnSpawner.node))),
)

it.effect("ignores an inaccessible MEDUSA_CONFIG_DIR", () =>
  Effect.gen(function* () {
    if (process.platform === "win32") return
    ...
    yield* withProcessEnv("MEDUSA_CONFIG_DIR", configDir, Config.use.get().pipe(provideInstanceEffect(dir)))
  }).pipe(Effect.provide(testInstanceStoreLayer), Effect.provide(LayerNode.compile(CrossSpawnSpawner.node))),
)

it.effect("creates a missing MEDUSA_CONFIG_DIR", () =>
  Effect.gen(function* () {
    const dir = yield* tmpdirScoped()
...
it.effect("installs dependencies in writable MEDUSA_CONFIG_DIR", () =>
  Effect.gen(function* () {
    const dir = yield* tmpdirScoped()
    const configDir = path.join(dir, "configdir")
    yield* FSUtil.use.ensureDir(configDir)
...
// Note: preload.ts sets MEDUSA_TEST_MANAGED_CONFIG which Global.Path.managedConfig uses
...
describe("MEDUSA_DISABLE_PROJECT_CONFIG", () => {
  it.instance(
  ...
  it.instance(
    ...
    () =>
      withProcessEnvs(
        { MEDUSA_CONFIG_DIR: undefined, MEDUSA_DISABLE_PROJECT_CONFIG: "true" },
        Effect.gen(function* () {
          const test = yield* TestInstance
  ...
  it.instance(
    "MEDUSA_CONFIG_DIR still works when flag is set",
    () =>
      Effect.gen(function* () {
        const configDir = yield* tmpdirScoped({ config: { model: "configdir/model" } })
        yield* withProcessEnvs(
          { MEDUSA_DISABLE_PROJECT_CONFIG: "true", MEDUSA_CONFIG_DIR: configDir },
          Effect.gen(function* () {
            const config = yield* Config.use.get()
...
// Regression for #28206: malformed MEDUSA_PERMISSION JSON used to crash
...
describe("MEDUSA_PERMISSION env var", () => {
  it.instance("does not crash when MEDUSA_PERMISSION contains invalid JSON", () =>
    withProcessEnv(
      "MEDUSA_PERMISSION",
      "{invalid",
      Effect.gen(function* () {
...
describe("MEDUSA_CONFIG_CONTENT token substitution", () => {
  it.instance("substitutes {env:} tokens in MEDUSA_CONFIG_CONTENT", () =>
    withProcessEnv(
      "TEST_CONFIG_VAR",
      "test_api_key_12345",
      withProcessEnv(
        "MEDUSA_CONFIG_CONTENT",
        JSON.stringify({
          $schema: "https://opencode.ai/config.json",
  ...
  it.instance("substitutes {file:} tokens in MEDUSA_CONFIG_CONTENT", () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
