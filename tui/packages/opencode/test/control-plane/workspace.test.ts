...
import { EventSequenceTable } from "@medusa-ai/core/event/sql"
...
import { WorkspaceV2 } from "@medusa-ai/core/workspace"
import { WorkspaceTable } from "@medusa-ai/core/control-plane/workspace.sql"
...
const originalEnv = {
  MEDUSA_AUTH_CONTENT: process.env.MEDUSA_AUTH_CONTENT,
  MEDUSA_EXPERIMENTAL_WORKSPACES: process.env.MEDUSA_EXPERIMENTAL_WORKSPACES,
...
beforeEach(() => {
  restoreEnv()
  process.env.MEDUSA_EXPERIMENTAL_WORKSPACES = "true"
})
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        const instance = yield* requireInstance
        const workspace = yield* Workspace.Service
        process.env.MEDUSA_AUTH_CONTENT = JSON.stringify({ test: { type: "api", key: "secret" } })
        ...
        expect(JSON.parse(recorded.calls.create[0].env.MEDUSA_AUTH_CONTENT ?? "{}")).toEqual({
          test: { type: "api", key: "secret" },
        })
        expect(recorded.calls.create[0].env.MEDUSA_WORKSPACE_ID).toBe(workspaceID)
        expect(recorded.calls.create[0].env.MEDUSA_EXPERIMENTAL_WORKSPACES).toBe("true")
