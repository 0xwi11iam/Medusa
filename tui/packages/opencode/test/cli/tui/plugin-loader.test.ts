...
import type { TuiAttentionSoundPack } from "@medusa-ai/plugin/tui"
...
const { allThemes, addTheme } = await import("@medusa-ai/tui/context/theme")
...
`,
  ...
  process.env.MEDUSA_PLUGIN_META_FILE = path.join(tmp.path, "plugin-meta.json")
  const config = createTuiResolvedConfig({
    plugin: [
...
`,
    ...
    const list = await Filesystem.readJson<Record<string, { themes?: Record<string, { dest: string }> }>>(
      process.env.MEDUSA_PLUGIN_META_FILE!,
    )
