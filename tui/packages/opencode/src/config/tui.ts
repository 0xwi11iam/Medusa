...
import { isRecord } from "@medusa-ai/tui/util/record"
...
import { TuiKeybind } from "@medusa-ai/tui/config/keybind"
import { InstallationLocal, InstallationVersion } from "@medusa-ai/core/installation/version"
import { makeRuntime } from "@medusa-ai/core/effect/runtime"
import { Filesystem } from "@/util/filesystem"
import { ConfigVariable } from "@/config/variable"
import { Npm } from "@medusa-ai/core/npm"
import { FormatError, FormatUnknownError } from "@/cli/error"
import { TuiConfig } from "@medusa-ai/tui/config"
...
const loadState = Effect.fn("TuiConfig.loadState")(function* (ctx: { directory: string }) {
  ...
  // folders between cwd and home, and MEDUSA_CONFIG_DIR.
  const directories = yield* ConfigPaths.directories(ctx.directory)
  yield* Effect.promise(() => migrateTuiConfig({ directories, cwd: ctx.directory }))

  const projectFiles = Flag.MEDUSA_DISABLE_PROJECT_CONFIG ? [] : yield* ConfigPaths.files("tui", ctx.directory)
  ...
  // 2. Explicit MEDUSA_TUI_CONFIG override, if set.
  if (Flag.MEDUSA_TUI_CONFIG) {
    const configFile = Flag.MEDUSA_TUI_CONFIG
  ...
  // 4. `.opencode` directories (and MEDUSA_CONFIG_DIR) discovered while
  // walking up the tree. Also returned below so callers can install plugin
  // dependencies from each location.
  const dirs = unique(directories).filter((dir) => dir.endsWith(".opencode") || dir === Flag.MEDUSA_CONFIG_DIR)

  for (const dir of dirs) {
    if (!dir.endsWith(".opencode") && dir !== Flag.MEDUSA_CONFIG_DIR) continue
