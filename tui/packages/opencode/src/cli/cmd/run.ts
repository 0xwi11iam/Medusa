import type { PermissionV1 } from "@medusa-ai/core/v1/permission"
...
import { createOpencodeClient, type OpencodeClient, type ToolPart } from "@medusa-ai/sdk/v2"
...
export const RunCommand = effectCmd({
  command: "run [message..]",
  describe: "run medusa with a message",
  ...
  builder: (yargs: Argv) =>
    yargs
      ...
      .option("attach", {
        type: "string",
        describe: "attach to a running medusa server (e.g., http://localhost:4096)",
      })
      .option("password", {
        alias: ["p"],
        type: "string",
        describe: "basic auth password (defaults to MEDUSA_SERVER_PASSWORD)",
      })
      .option("username", {
        alias: ["u"],
        type: "string",
        describe: "basic auth username (defaults to MEDUSA_SERVER_USERNAME or 'opencode')",
      })
      .option("dir", {
        type: "string",
