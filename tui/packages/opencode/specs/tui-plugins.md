...
- Author package entrypoint is `@medusa-ai/plugin/tui`.
...
- Import types from `@medusa-ai/plugin/tui`.
- `@medusa-ai/plugin` exports `./tui` and declares optional peer deps on `@opentui/core` and `@opentui/solid`.

Minimal module shape:

```tsx
/** @jsxImportSource @opentui/solid */
import type { TuiPlugin, TuiPluginModule } from "@medusa-ai/plugin/tui"
...
When `plugin` entries exist in a writable `.opencode` dir or `MEDUSA_CONFIG_DIR`, OpenCode installs `@medusa-ai/plugin` into that dir and writes:
...
That is what makes local config-scoped plugins able to import `@medusa-ai/plugin/tui`.
...
- For generic config-to-bindings helpers, import `createBindingLookup` from `@medusa-ai/plugin/tui`.
...
- `--pure` / `MEDUSA_PURE` skips external TUI plugins only.
