...
import { FileSystemWatcher } from "@medusa-ai/schema/filesystem-watcher"
...
declare const MEDUSA_LIBC: string | undefined
...
const watcher = lazy((): typeof import("@parcel/watcher") | undefined => {
  try {
    const libc = typeof MEDUSA_LIBC === "undefined" ? undefined : MEDUSA_LIBC
...
const layer = Layer.effect(
  Service,
  Effect.gen(function* () {
    if (yield* Flag.MEDUSA_EXPERIMENTAL_DISABLE_FILEWATCHER) return Service.of({})
    ...
    if (location.vcs && (yield* Flag.MEDUSA_EXPERIMENTAL_FILEWATCHER)) {
