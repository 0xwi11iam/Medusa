...
import type { IntegrationOAuthMethodRegistration } from "@medusa-ai/plugin/v2/effect/integration"
import { define } from "@medusa-ai/plugin/v2/effect/plugin"
import type { CredentialValue } from "@medusa-ai/sdk/v2/types"
...
export const OpencodePlugin = define<HttpClient.HttpClient | EventV2.Service | Scope.Scope>({
  ...
  effect: Effect.fn(function* (ctx) {
    ...
    yield* ctx.catalog.transform((catalog) => {
      ...
      if (!item) return
      const hasKey = Boolean(process.env.MEDUSA_API_KEY || connected || item.provider.request.body.apiKey)
